 ...
 *
 * L9 stage of the qa-minimal ladder.
 *
...
async function run(ctx) {
  ...
  const question = process.env.QA_QUESTION
    || "Build a tiny CLI in this workspace. First record a todo list with one task per step. Then: create hello.py that prints \"hello\"; run it; add a --name flag that greets a named person; run it again; add a --count flag that repeats the greeting N times; run it once more. Finally summarize what you built and how you verified each step."
