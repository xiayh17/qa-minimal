 ...
 *
 * L7 stage of the qa-minimal ladder.
 *
...
async function run(ctx) {
  ...
  const question = process.env.QA_QUESTION
    || "Fix the divide-by-zero bug in src/calculator.js. Track the work with the todo_write tool: create a list (reproduce the bug, fix it, run the tests to verify), mark each task in_progress while you work on it and completed when done."
