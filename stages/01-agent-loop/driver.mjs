/**
 * Stage 01 driver: the first Agent.
 *
 * inject ['llm', 'agents'] — the LLM registry (to wait for the adapter route)
 * and the agent registry (to create and drive the agent).
 *
 * Flow: wait for loader settlement → create an agent → send one user message
 * → wait for the turn to complete → print the final assistant text + session id.
 *
 * --trace: live-stream every session/event as it fires, so the user can SEE
 * the turn/step boundary that Stage 00 lacks.
 */
